    ...
    stage('Build image') {
      steps{
        script {
          ...
          sh "docker build --no-cache -t 192.168.100.12/bb-edu/commerce-order:v2-$BUILD_NUMBER ."
    ...
    stage('Push Image') {
      ...
      steps{
        script {
          ...
          sh "docker push 192.168.100.12/bb-edu/commerce-order:v2-$BUILD_NUMBER"
          }
        echo "Push Image END"
      }
    }
    ...
    stage('Deploy App') {
      steps {
        script {
          ...
          sh "/usr/local/bin/kubectl --kubeconfig=/home/jenkins/acloud-client.conf set image deployments/commerce-order order-cotainer=192.168.100.12/bb-edu/commerce-order:v2-$BUILD_NUMBER -n bb-edu"
          echo "Deploy App END"
        }
      }
    }
